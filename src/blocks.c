...
/*   Updated: 2026/04/02 04:27:13 by molasz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_block	*create_block(t_zone *zone, t_block *prev, size_t size)
{
	...
	if (prev)
		block = (t_block *)((char *)prev + BLOCK_SIZE + align16(prev->size));
	else
	...
	if (prev)
		...
		zone->blocks = block;
	zone->used += align16(size) + BLOCK_SIZE;
	return (block);
}
...
void	fractionate_block(t_block *old, size_t size)
{
	...
	block = (t_block *)((char *)(old + 1) + align16(size));
	block->size = align16(old->size) - align16(size) - BLOCK_SIZE;
	block->free = 1;
	block->next = old->next;
	old->next = block;
	old->size = size;
...
t_block	*find_block(t_zone *zone, size_t size)
{
	...
	while (block)
	{
		if (block->free && align16(block->size) >= size)
		{
			if (align16(block->size) >= align16(size) + BLOCK_SIZE + 16)
